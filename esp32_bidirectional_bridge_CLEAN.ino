...
const unsigned long PRINT_INTERVAL = 10000; // Print status every 10s
...
bool waitingForButtonActivation = false;   // NEW: Flag to wait for button press after motor move
...
void loop() {
  // PRIORITY 1: Check button for sensor activation (if waiting)
  if (waitingForButtonActivation) {
    checkButtonForSensorActivation();
  }
  
  // PRIORITY 2: Check for physical moves ONLY if sensors are active
  if (!motorMovementActive && !waitingForButtonActivation) {
    checkForPhysicalMoves();
  } 
  // PRIORITY 3: Check motor status if motors are active
  else if (motorMovementActive) {
    checkMotorMovementStatus();
  }
  
  // PRIORITY 4: Check WiFi connection (non-blocking)
  checkWiFiConnectionNonBlocking();
  
  // PRIORITY 5: Poll for web/AI moves only if WiFi is stable
...
// ==========================================
// 🔘 CHECK BUTTON FOR SENSOR ACTIVATION
// ==========================================
void checkButtonForSensorActivation() {
  ...
  if (lastButtonState == HIGH && currentButtonState == LOW) {
    ...
    if (currentTime - lastButtonPress > BUTTON_DEBOUNCE) {
      ...
      if (waitingForButtonActivation) {
        Serial.println();
        Serial.println("🔘 BUTTON PRESSED - Activating sensor detection!");
        Serial.println("================================================");
        
        // Force reset all sensor states to ignore position differences
        Serial.println("🔄 Resetting sensor baseline after motor movement...");
        
        // Do a quick scan to get current sensor states
        for (int i = 0; i < 10; i++) {
          scanAndDebounceBoard();
          delay(20);
        }
        ...
        waitingForButtonActivation = false;
...
// ==========================================
// 🎯 ENHANCED MOVEMENT DETECTION (UPDATED FOR BUTTON CONTROL)
// ==========================================
void detectMovements() {
  if (!boardInitialized) return;
  
  // CRITICAL: Block sensor detection during motor movements OR waiting for button
  if (motorMovementActive || waitingForButtonActivation) {
    // Only print status occasionally to avoid spam
    static unsigned long lastStatusMessage = 0;
    if (millis() - lastStatusMessage > 5000) { // Every 5 seconds
      if (waitingForButtonActivation) {
        Serial.println("🔘 Waiting for BUTTON press to activate sensor detection...");
      } else {
        Serial.println("🤖 Motor movement active - sensor detection disabled");
      }
      ...
      if (sensors[row][col].hasChanged) {
        ...
        if (!previousState && currentState) {
          if (!pendingMove.isActive) {
            ...
            Serial.println("🎯 PHYSICAL PLAYER - PIECE LIFTED: " + fromSquare);
        ...
        // Piece placed (magnet detected: 1 -> 0)
          ...
          if (pendingMove.isActive) {
            ...
            if (!(pendingMove.fromRow == row && pendingMove.fromCol == col)) {
              Serial.println("================================================");
              Serial.println("🎯 PHYSICAL PLAYER MOVE DETECTED!");
              Serial.println("================================================");
              Serial.print("📍 FROM: ");
...
// ==========================================
// 🤖 CHECK MOTOR MOVEMENT STATUS (UPDATED FOR BUTTON CONTROL)
// ==========================================
void checkMotorMovementStatus() {
  unsigned long currentTime = millis();
  
  // Check for motor movement timeout
  if (currentTime - motorMovementStartTime > MOTOR_MOVEMENT_TIMEOUT) {
    Serial.println("⏰ Motor movement timeout - enabling button activation");
    waitingForButtonActivation = true; // Still require button press even on timeout
    return;
  }
  
  // Check if Arduino has finished moving
  if (ARDUINO_SERIAL.available()) {
    String msg = ARDUINO_SERIAL.readStringUntil('\n');
    msg.trim();
    
    if (msg == "MOVE_COMPLETED" || msg == "Move completed. Ready for next move.") {
      Serial.println();
      Serial.println("************************************************");
      Serial.println("✅ ARDUINO MOVEMENT COMPLETED!");
      Serial.println("************************************************");
      Serial.println("🔘 Press BUTTON on GPIO19 to activate sensor detection");
      Serial.println("⏳ Sensor detection will remain DISABLED until button press");
      Serial.println("💡 This allows you to prepare for your physical move");
      Serial.println("************************************************");
      
      // Don't enable sensors yet - wait for button press
      waitingForButtonActivation = true;
      // Keep motorMovementActive = true until button is pressed
    }
  }
}
...
void printBoardStatus() {
  ...
  if (waitingForButtonActivation) {
    Serial.println("🔘 WAITING FOR BUTTON PRESS to activate sensor detection");
    Serial.println("⏳ Press button on GPIO19 when ready for physical moves");
  } else if (motorMovementActive) {
    unsigned long elapsed = (millis() - motorMovementStartTime) / 1000;
    Serial.print("🤖 MOTOR MOVEMENT ACTIVE (");
    Serial.print(elapsed);
    Serial.println("s) - Sensor detection DISABLED");
  } else {
  ...
  if (pendingMove.isActive && !motorMovementActive && !waitingForButtonActivation) {
    String fromSquare = squareName(pendingMove.fromRow, pendingMove.fromCol);
    unsigned long elapsed = (millis() - pendingMove.startTime) / 1000;
    Serial.print("PENDING PHYSICAL MOVE FROM: ");
...
void sendMoveToArduino(String move) {
  ...
  // CRITICAL FIX: Block sensor detection during motor movement
  ...
  Serial.println("⏳ Physical move detection paused until motor completes...");
...
void handleArduinoMotorSequence() {
  ...
  while (millis() - startTime < timeout && !moveCompleted) {
    if (ARDUINO_SERIAL.available()) {
      ...
      } else if (msg == "MOVE_COMPLETED" || msg == "Move completed. Ready for next move.") {
  ...
  if (!moveCompleted) {
    ...
    // Force re-enable sensor detection on timeout
    motorMovementActive = false;
    DEBUG_SERIAL.println("🎯 Sensor detection force re-enabled due to timeout");
  }
}
