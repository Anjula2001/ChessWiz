...
// IGNORE WEB MOVE DETECTION - Prevent false physical moves after motor execution
struct WebMoveIgnore {
  byte fromRow, fromCol;
  byte toRow, toCol;
  unsigned long executeTime;
  bool isActive;
};
WebMoveIgnore webMoveToIgnore = {0, 0, 0, 0, 0, false};
...
void parseSquare(String square, byte &row, byte &col) {
  if (square.length() >= 2) {
    col = square.charAt(0) - 'a';  // 'a' = 0, 'b' = 1, etc.
    row = square.charAt(1) - '1';  // '1' = 0, '2' = 1, etc.
  }
}
...
void connectToWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    DEBUG_PRINT("👁 Monitoring hall sensors continuously...");
...
void checkWiFiConnection() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (networkStable) {
      DEBUG_PRINT("⚠ WiFi connection lost - ESP32 will auto-reconnect in background");
      networkStable = false;
    }
  }
}
...
void checkForWebMovesTask() {
  ...
  if (httpCode > 0) {
    ...
    if (consecutiveErrors >= maxConsecutiveErrors) {
      DEBUG_PRINT("⚠ Multiple HTTP errors - but continuing operation");
...
void sendPhysicalMoveTask(String move) {
  if (WiFi.status() != WL_CONNECTED) {
    if (xSemaphoreTake(serialMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
      Serial.println("⚠ WiFi not connected - move buffered: " + move);
  ...
  if (move == lastSentMove) {
    if (xSemaphoreTake(serialMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
      Serial.println("⚠ Duplicate move blocked: " + move);
      xSemaphoreGive(serialMutex);
    }
    return;
  }
...
void processWebMoveTask(String move, String source) {
  ...
  // Parse the move to get from/to squares for ignoring sensor detections
  if (move.length() >= 5 && move.charAt(2) == '-') {
    String fromSquare = move.substring(0, 2);
    String toSquare = move.substring(3, 5);
    
    // Store the squares to ignore when sensors are re-enabled
    parseSquare(fromSquare, webMoveToIgnore.fromRow, webMoveToIgnore.fromCol);
    parseSquare(toSquare, webMoveToIgnore.toRow, webMoveToIgnore.toCol);
    webMoveToIgnore.executeTime = millis();
    webMoveToIgnore.isActive = true;
    
    if (xSemaphoreTake(serialMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
      Serial.println("🚫 IGNORING SENSOR CHANGES: " + fromSquare + " and " + toSquare);
      xSemaphoreGive(serialMutex);
    }
  }
      ...
      if (xSemaphoreTake(serialMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        ...
        if (sensorsDisabled) {
          ...
          if (xSemaphoreTake(sensorMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
            ...
            // Reset all sensor change flags and set current state as baseline
            for (byte row = 0; row < 8; row++) {
              for (byte col = 0; col < 8; col++) {
                sensors[row][col].previousStable = sensors[row][col].stableState;
            ...
            // Set ignore timeout (ignore web move related changes for 3 seconds)
            if (webMoveToIgnore.isActive) {
              webMoveToIgnore.executeTime = millis();
              Serial.println("⏰ Ignoring web move squares for 3 seconds after button press");
            }
          ...
          Serial.println("ℹ Sensors already active");
        }
        xSemaphoreGive(serialMutex);
      }
    }
...
void detectMovementsTask() {
  ...
  // Clear web move ignore after 3 seconds
  if (webMoveToIgnore.isActive && (currentTime - webMoveToIgnore.executeTime > 3000)) {
    webMoveToIgnore.isActive = false;
    if (xSemaphoreTake(serialMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
      Serial.println("✅ Web move ignore period ended - normal detection resumed");
      xSemaphoreGive(serialMutex);
    }
  }
  ...
  for (byte row = 0; row < 8; row++) {
    for (byte col = 0; col < 8; col++) {
      if (sensors[row][col].hasChanged) {
        ...
        // CHECK: Is this square part of the recent web move? If so, ignore it
        bool shouldIgnoreSquare = false;
        if (webMoveToIgnore.isActive) {
          if ((row == webMoveToIgnore.fromRow && col == webMoveToIgnore.fromCol) ||
              (row == webMoveToIgnore.toRow && col == webMoveToIgnore.toCol)) {
            shouldIgnoreSquare = true;
            String squareToIgnore = squareName(row, col);
            if (xSemaphoreTake(serialMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
              Serial.println("🚫 IGNORING: " + squareToIgnore + " (web move square)");
              xSemaphoreGive(serialMutex);
            }
          }
        }
        
        if (!shouldIgnoreSquare) {
...
void printBoardStatusTask() {
  if (xSemaphoreTake(serialMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
    ...
    // Show web move ignore status
    if (webMoveToIgnore.isActive) {
      String fromSquare = squareName(webMoveToIgnore.fromRow, webMoveToIgnore.fromCol);
      String toSquare = squareName(webMoveToIgnore.toRow, webMoveToIgnore.toCol);
      unsigned long timeLeft = 3000 - (millis() - webMoveToIgnore.executeTime);
      Serial.println("🚫 IGNORING: " + fromSquare + " & " + toSquare + " (" + timeLeft + "ms left)");
    }
  ...
  if (lastResetButtonState == HIGH && currentResetButtonState == LOW) {
    ...
    if (currentTime - lastResetButtonPress > RESET_BUTTON_DEBOUNCE) {
      ...
      if (ARDUINO_SERIAL.available()) {
        ...
        if (xSemaphoreTake(serialMutex, pdMS_TO_TICKS(50)) == pdTRUE) {
          Serial.println("⚠ Arduino did not respond - may still be restarting");
          xSemaphoreGive(serialMutex);
        }
      }
