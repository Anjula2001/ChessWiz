...
function MultiplayerChess({ playerColor, onBackToHome }) {
  ...
  const makeMove = (moveStr, isRemoteMove = false) => {
    try {
      ...
      try {
        ...
        if (fromToMatch) {
          ...
          // Try without dashes (e.g., "e2e4", "g1f3")
          const fromToNoDashMatch = moveStr.match(/^([a-h][1-8])([a-h][1-8])([qrbnQRBN]?)$/);
          if (fromToNoDashMatch) {
            const [, from, to, promotion] = fromToNoDashMatch;
            moveAttempt = game.move({
              from: from,
              to: to,
              promotion: promotion?.toLowerCase() || 'q'
  ...
  const handleDragStart = (e, square) => {
    ...
    if (!piece || piece[0] !== playerColorCode || game.turn() !== playerColorCode) {
      console.log(`Drag prevented: piece=${piece}, piece color=${piece?.[0]}, player color=${playerColorCode}, turn=${game.turn()}`);
      e.preventDefault();
      return;
    }
    
    console.log(`Drag started: ${square} with piece ${piece}`);
  ...
  const handleDrop = (e, targetSquare) => {
    ...
    try {
      ...
      if (moveAttempt) {
        console.log(`Move successful: ${moveAttempt.san}`);
        const newGame = new Chess(game.fen());
        setGame(newGame);
        ...
        console.log(`Invalid move from ${fromSquare} to ${targetSquare}`);
      }
    } catch (error) {
  ...
  const startNewGame = () => {
    ...
    setMoveHistory([]);
    setMoveInput('');
    // Clear physical move alerts when starting a new game
    setLastPhysicalMove(null);
    setPhysicalMoveApplied(false);
    updateBoardState(newGame);
    
    // Notify backend that a new game has started to clear physical move state
    if (connected) {
      socket.emit('newGame', roomId);
      console.log('🔄 Notified backend of new game start');
    }
  };
  ...
  useEffect(() => {
    ...
    // Clear any physical move state on component mount
    setLastPhysicalMove(null);
    setPhysicalMoveApplied(false);
    ...
    const transferStatusInterval = setInterval(() => {
      if (!lastPhysicalMove) {
        console.log('%c📡 TRANSFER STATUS 📡', 'background: #9b59b6; color: white; padding: 5px; border-radius: 5px; font-weight: bold; font-size: 14px');
    ...
    for (let i = 0; i < 64; i++) {
      ...
      squares.push(
        ...
        >
          {piece && (
            <div 
              className={`piece ${piece[0] === (actualPlayerColor === 'white' ? 'w' : 'b') ? 'draggable' : ''}`}
              draggable={piece[0] === (actualPlayerColor === 'white' ? 'w' : 'b')}
  ...
  useEffect(() => {
    ...
    // Show initial transfer status when component mounts
    console.log('%c📡 TRANSFER STATUS 📡', 'background: #9b59b6; color: white; padding: 5px; border-radius: 5px; font-weight: bold; font-size: 14px');
    console.log('transfering move is - not received');
    
    // Handle incoming moves from other players
    socket.on('moveMade', ({ move, fromESP }) => {
      console.log(`Received move from ${fromESP ? 'ESP device' : 'other player'}: ${move}`);
      makeMove(move, true); // true means this is a remote move
      if (fromESP) {
        setLastPhysicalMove(move);
      }
    });
    
    // Handle physical moves from ESP device
    socket.on('physicalMove', ({ move, playerSide, source, timestamp }) => {
      // Show transfer status immediately when move is received
      console.log('%c📡 TRANSFER STATUS 📡', 'background: #9b59b6; color: white; padding: 5px; border-radius: 5px; font-weight: bold; font-size: 14px');
      console.log(`transfering move is - ${move}`);
      ...
      console.log('%c🎮 PHYSICAL MOVE RECEIVED 🎮', 'background: #3498db; color: white; padding: 5px; border-radius: 5px; font-weight: bold; font-size: 14px');
      ...
      if (isStaleMove) {
        console.log('%c⏰ IGNORING STALE MOVE ⏰', 'background: #f39c12; color: white; padding: 5px; border-radius: 5px; font-weight: bold; font-size: 14px');
        console.log(`Move ${move} is stale (received from server cache, not a real-time event)`);
        console.log('%c📡 TRANSFER STATUS 📡', 'background: #9b59b6; color: white; padding: 5px; border-radius: 5px; font-weight: bold; font-size: 14px');
        console.log('transfering move is - not received (stale move ignored)');
        return;
      }
      ...
      const topPlayerColor = actualPlayerColor === 'white' ? 'black' : 'white';
      const topPlayerColorCode = topPlayerColor === 'white' ? 'w' : 'b';
      
      // Check whose turn it is currently
      const currentTurn = game.turn();
      
      console.log(`🎯 ESP Move Analysis: Frontend Player: ${actualPlayerColor} | Top Player: ${topPlayerColor} | Current Turn: ${currentTurn === 'w' ? 'white' : 'black'}`);
      console.log(`📋 Move Logic: ESP move "${move}" is for top player (${topPlayerColor}). Will apply only if it's ${topPlayerColor}'s turn.`);
      
      // ESP moves are ALWAYS for the top player
      // Only apply the move if it's currently the top player's turn
      if (currentTurn === topPlayerColorCode) {
        console.log(`✅ Applying physical move for top player (${topPlayerColor}): ${move}`);
        
        // Handle moves in format "e2-e4" by converting to object format
        let from, to;
        if (move.includes('-')) {
          [from, to] = move.split('-');
          ...
          console.log('%c❌ INVALID MOVE FORMAT ❌', 'background: #e74c3c; color: white; padding: 5px; border-radius: 5px; font-weight: bold; font-size: 14px');
          console.log(`🚫 Format error: ${move} - Move should be in format "e2-e4" or "e2e4"`);
          setPhysicalMoveApplied(false);
          
          // Show transfer status for invalid format
          console.log('%c📡 TRANSFER STATUS 📡', 'background: #9b59b6; color: white; padding: 5px; border-radius: 5px; font-weight: bold; font-size: 14px');
          console.log(`transfering move is - ${move} (invalid format)`);
          return;
        }
        
        // Try to make the move directly using from-to object notation
        try {
          const moveAttempt = game.move({
            from: from,
            to: to,
          ...
          if (moveAttempt) {
            ...
            console.log('%c✅ PHYSICAL MOVE APPLIED ✅', 'background: #27ae60; color: white; padding: 5px; border-radius: 5px; font-weight: bold; font-size: 14px');
            console.log(`📊 Details: Move ${move} | Top Player: ${topPlayerColor} | Piece: ${moveAttempt.piece} | From: ${moveAttempt.from} | To: ${moveAttempt.to} | Capture: ${moveAttempt.captured ? 'Yes' : 'No'}`);
            
            // Show final transfer status after successful application
            console.log('%c📡 TRANSFER STATUS 📡', 'background: #9b59b6; color: white; padding: 5px; border-radius: 5px; font-weight: bold; font-size: 14px');
            console.log(`transfering move is - ${move} (successfully applied)`);
            ...
            console.log('❌ Physical move failed to apply:', move);
            setPhysicalMoveApplied(false);
            
            // Show transfer status for failed move
            console.log('%c📡 TRANSFER STATUS 📡', 'background: #9b59b6; color: white; padding: 5px; border-radius: 5px; font-weight: bold; font-size: 14px');
            console.log(`transfering move is - ${move} (failed to apply - invalid move)`);
          }
        } catch (error) {
          console.log('❌ Error applying physical move:', move, error.message);
          setPhysicalMoveApplied(false);
          
          // Show transfer status for error
          console.log('%c📡 TRANSFER STATUS 📡', 'background: #9b59b6; color: white; padding: 5px; border-radius: 5px; font-weight: bold; font-size: 14px');
          console.log(`transfering move is - ${move} (error occurred: ${error.message})`);
        }
      } else {
        console.log('%c❌ MOVE REJECTED - WRONG TURN ❌', 'background: #e74c3c; color: white; padding: 5px; border-radius: 5px; font-weight: bold; font-size: 14px');
        console.log(`🔄 Current turn: ${currentTurn === 'w' ? 'white' : 'black'} | Top player: ${topPlayerColor} | Frontend player: ${actualPlayerColor}`);
        console.log(`🚫 Physical move "${move}" rejected because it's not the top player's turn`);
        setPhysicalMoveApplied(false);
        
        // Show transfer status for rejected move
        console.log('%c📡 TRANSFER STATUS 📡', 'background: #9b59b6; color: white; padding: 5px; border-radius: 5px; font-weight: bold; font-size: 14px');
        console.log(`transfering move is - ${move} (rejected - wrong turn)`);
      }
    ...
    socket.on('gameState', (gameState) => {
      ...
      if (gameState.currentFen && gameState.currentFen !== game.fen()) {
        try {
    ...
    socket.on('newGameStarted', ({ roomId: gameRoomId }) => {
      console.log('Received new game started event from backend for room:', gameRoomId);
      // Clear physical move alerts when backend confirms new game
      setLastPhysicalMove(null);
      setPhysicalMoveApplied(false);
    });
    ...
    return () => {
      ...
      // Don't disconnect completely as socket might be used elsewhere
    };
  }, [game, roomId]);

  // Calculate recent moves for display
